...
\usepackage{latex2pydata}                       % для minted
\usepackage[normalem]{ulem}						% стилизация подчеркиваний
\usepackage{microtype}							% тонкая стилизация текста
\usepackage{enumitem}							% настройка списков
\usepackage[cachedir=./_minted-cache]{minted}	% продвинутый листинг
...
\setlist[itemize]{
    label=---~,		% длинное тире
    labelwidth=-0.5em,	% ширина маркера
    labelsep=0.25em,	% расстояние между маркером и текстом
    leftmargin=1.25cm
}
...
\tcbset{
  mylisting base/.style={
    enhanced,
    before skip=10pt,
    arc=0mm,
    boxrule=1pt,
	enlarge top by=1.8ex, % опустить рамку с кодом
    listing only,
    ...
    colframe=black,
    breakable,
    title={\strut \tcbtitletext}, % только плейсхолдер
    ...
    overlay middle={
      ...
      \node[anchor=south west,inner sep=0pt] at (frame.north west)
          {Продолжение листинга \thetcbcounter};
    },
    overlay last={
      \draw[line width=.5pt] (frame.north west)--(frame.north east);
      \node[anchor=south west,inner sep=0pt] at (frame.north west)
...
\newtcbinputlisting[% язык файл label подпись
  auto counter,
  ...
  minted language=#1,
  listing file={#2},
  title={\strut Листинг \thetcbcounter{} --- #4},
  label=lst:#3
}
% \Code[java]{code/main.java}{L1}{Пример простой функции}
...
% \GlobalCode[java]{code/main.java}{L1}{Пример простой функции}
...
% \AppendixCode[java]{code/main.java}{L1}{Пример простой функции}
...
\begin{minted}[linenos,breaklines,style=algol_nu]{python}
def hello():
    print("Hello, world!")
\end{minted}
%
