...
{\normalfont\fontsize{18}{21}\bfseries\centering}
{\thesubsection}{0.5em}{}				% расстояние между № и названием (21 = 14 * 1.5 === полуторный интервал)
\titlespacing\subsection{\parindent}{\parskip}{1.5ex}
...
% Рисунок в разделе
\NewDocumentCommand{\Fig}{ O{1} m m }{% [размер] файл/lable подпись
    \counterwithin{figure}{subsection}%
    \begin{figure}[!htbp]
        \centering
...
% Листинг в разделе
\NewDocumentCommand{\Code}{ O{C} m m m }{% [язык] файл lable подпись
    \counterwithin{lstlisting}{subsection}%
    \lstinputlisting[
        language=#1,
        caption={#4},
...
% Таблица в разделе
\NewDocumentCommand{\Table}{ m m m }{% таблица lable подпись
    \counterwithin{table}{subsection}%
    \begin{table}[!htbp]
        \centering
		\caption{#3}
