...
\usepackage{minted}								% продвинутый листинг
...
\setlist[enumerate]{
    label={\arabic*)},
    labelwidth=-0.5em,	% ширина маркера
...
%-----------------------------------------------------------------------------------
% Те же команды, но для длинных таблиц

% Таблица в разделе
\makeatletter
\NewDocumentCommand{\TableL}{ m m m }{% colspec, label, caption
    \counterwithin{table}{subsection}%
    \begin{longtable}{#1}
        \caption{#3}\label{tab:#2} \\
        \endfirsthead

        \multicolumn{\LT@cols}{@{}p{\linewidth}}%
        {Продолжение таблицы \ref{tab:#2}}\\
		\hline
        \endhead
}
\makeatother
...
% Таблица со сквозной нумерацией
\makeatletter
\NewDocumentCommand{\GlobalTableL}{ m m m }{% colspec, label, caption
    \counterwithout{table}{subsection}%
    \counterwithout{table}{section}%
    \begin{longtable}{#1}
        \caption{#3}\label{tab:#2} \\
...
% Таблица в приложении
\makeatletter
\NewDocumentCommand{\AppendixTableL}{ m m m }{% colspec, label, caption
    \counterwithin{table}{section}%
    \begin{longtable}{#1}
        \caption{#3}\label{tab:#2} \\
        \endfirsthead

        \multicolumn{\LT@cols}{@{}p{\linewidth}}% растягивает последнюю колонку
        {Продолжение таблицы \ref{tab:#2}}\\
		\hline
        \endhead
}
...
% Альтернатива, но прилется подбирать значение в ручную
%\multicolumn{\LT@cols}{@{}l}{%
%  \makebox[0pt][l]{%
%	\hspace*{-\dimexpr10mm}%
%	Продолжение таблицы \ref{tab:main}%
%  }%
%}\\
...
%-----------------------------------------------------------------------------------
\iffalse
% Шаблон длинной таблицы
\AppendixTableL{|l|c|r|}{params}{Параметры системы}
\hline
№ & Параметр & Значение \\
\hline
1 & Скорость & 10 \\ \hline
...
% Подключаем tcolorbox с поддержкой листингов
% listings — для совместимости, minted — для подсветки через Pygments
\RequirePackage[listings]{tcolorbox}
\tcbuselibrary{listings,breakable,minted,skins}

% Уменьшаем размер номеров строк (FancyVerbLine используется minted'ом)
\renewcommand\theFancyVerbLine{\scriptsize\arabic{FancyVerbLine}}


% Определение команды \mylisting
% [auto counter]        — автоматическая нумерация листингов
% [number within=section] — нумерация вида <section>.<listing>
% Аргументы:
%   #1 — язык (java, python, sql и т.п.)
%   #2 — путь к файлу с кодом
%   #3 — заголовок листинга
%   #4 — label (без префикса lst:)
\newtcbinputlisting[auto counter,number within=section]{\mylisting}[4][]{
  enhanced,						% включает расширенные возможности tcolorbox
  before skip=10pt,				% вертикальный отступ перед листингом
  arc=0mm,						% прямые углы рамки (без скруглений)
  boxrule=1pt,					% толщина рамки
  listing only,					% внутри только листинг, без текста
  listing file={#2},			% файл с исходным кодом
  listing engine=minted,		% используем minted как движок подсветки
  minted language=#1,			% язык подсветки
  minted options = {
    linenos,					% нумерация строк
	xleftmargin=5pt,
    breaklines=true,			% перенос длинных строк
    fontsize=\small\ttfamily,	% размер и семейство шрифта
    numbersep=1.5mm,			% расстояние между номерами строк и кодом
    firstnumber=1,				% начальный номер строки
	style=bw
  },
  colback=white,				% цвет фона
  colframe=black,				% цвет рамки
  breakable,					% разрешить перенос листинга на несколько страниц
  title={\strut Листинг \thetcbcounter{} — #3},
  ...
  before upper={\vspace{\tcboxedtitleheight}},
  coltitle=black,				% цвет текста заголовка
  colbacktitle=white,			% фон заголовка
  % Поведение рамки и подписи при переносе на несколько страниц
  % Первая страница листинга
  overlay first={
    \draw[line width=.5pt]
      (frame.south west)--(frame.south east);	% нижняя линия
  },
  ...
  overlay middle={
    \draw[line width=.5pt]
      (frame.south west)--(frame.south east);	% нижняя линия
    \draw[line width=.5pt]
      (frame.north west)--(frame.north east);	% верхняя линия
    \node[anchor=south west]
      at (frame.north west)
      {\hspace{-0.4em}Продолжение листинга \thetcbcounter};
  },
  % Последняя страница
  overlay last={
    \draw[line width=.5pt]
